...
const PrescriptionPage = () => {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      if (session?.user?.email) {
        ...
        setIsStaffUser(session.user.email === 'staff@dantsri.com');
      }
    };
  ...
  const [selectedToothNumber, setSelectedToothNumber] = useState<string | number | null>(null);
  ...
  const handleAddTooth = () => {
    if (selectedQuadrant && selectedToothNumber && selectedDisease) {
      // Convert letter teeth to numbers (A=9, B=10, etc.)
      let toothId: number;
      if (typeof selectedToothNumber === 'string' && /[A-E]/.test(selectedToothNumber)) {
        const letterMap: { [key: string]: number } = { 'A': 9, 'B': 10, 'C': 11, 'D': 12, 'E': 13 };
        toothId = parseInt(`${selectedQuadrant}${letterMap[selectedToothNumber]}`);
      } else {
        toothId = parseInt(`${selectedQuadrant}${selectedToothNumber}`);
      }
  ...
  }, []); // This effect should only run once on mount since it's initializing data
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => setSelectedToothNumber(e.target.value)}
